...
| 账户与权限 | 分为管理员账户和普通用户账户；管理员可创建账户、审核申请和管理 CA，普通用户只能申请、查看和撤销自己的证书 |
| 用户证书管理 | 采用签名证书 + 加密证书双证书机制，支持证书详情、下载和状态查询 |
| 多级证书 | 管理员可签发中间 CA，用户双证书可由根 CA 或中间 CA 签发 |
| 证书链验证 | 支持输入 PEM 或序列号验证，可选择查看完整证书链，并验证签发关系、CA 约束和有效期 |
| CRL 管理 | 用户提交证书撤销申请；管理员审核通过后撤销证书并生成 CRL |
...
管理员可在“账户管理”中创建普通用户账户。普通用户登录后只能访问自己的证书申请、证书列表、撤销和验证功能。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/api/cert/issue` | 管理员直接签发双证书或中间 CA |
...
| `POST` | `/api/cert/verify` | 按 PEM 或序列号验证证书链，可返回完整链 |
...
- 管理员账户：创建管理员/普通用户账户，初始化和管理 CA，签发中间 CA，审核用户证书申请，生成 CRL，查看系统设置。
- 普通用户账户：提交签名+加密双证书申请，查看自己的证书，下载自己的证书链，提交证书撤销申请，按 PEM 或序列号验证证书。
- 管理员审核中心：统一处理证书签发申请和证书撤销申请；撤销申请通过后证书才会进入撤销状态，并在 CRL 发布时体现。
- 证书签发默认采用双证书机制：一次申请审核通过后会同时生成签名证书和加密证书。
- 可选签名算法：`SM3WITHSM2`、`SHA256WITHECDSA`、`SHA384WITHECDSA`、`SHA512WITHECDSA`。
- 可选加密算法：`SM2`、`ECIES-SECP256R1`。
- 多级证书链：管理员可先签发 `intermediate_ca`，再在签发双证书时填写中间 CA 序列号作为上级 CA。
